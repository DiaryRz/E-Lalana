...
<head>
    <%
        Type appelType = new Type();
        List<Type> listeType = appelType.SelectType();
        ...
        List<Personnel> ListePersonnel = appelPersonnel.SelectPersonnel();
    %>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="container-fluid my-5">
            ...
            <div class="row justify-content-center w-100">
                <div class="col-md-12">
                    <h3>Personnes necessaire pour une fabrication de route</h3>
                    <form action="InsertionEmployeTravaillant.jsp" method="post" class="question-form w-100">
                        <div class="mb-3">
                            Type d'employer:
                            <select name="idPersonnel" class="form-control" required>
                                <% for (int i = 0; i < ListePersonnel.size(); i++) { %>
                                    <option value="<%=ListePersonnel.get(i).getIdPersonnel() %>"><%= ListePersonnel.get(i).getNomPersonnel() %></option>
                                <% } %>
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            Salaire par heure:
                            <input type="number" name="salaire" class="form-control" placeholder="Votre salaire" required>
                        </div>
                        <div class="mb-3">
                            Nombre de mettre carre pas heure:
                            <input type="number" name="mettre" class="form-control" placeholder="Nombre de mettre carre" required>
                        </div>
                        <div class="mb-3">
                            Nombre de personne necessaire:
                            <input type="number" name="nombre" class="form-control" placeholder="Votre quantite" required>
                        </div>
                        <div class="d-grid">
                    ...
                    <%
                        if( request.getParameter("mettre") != null && request.getParameter("idPersonnel") != null && request.getParameter("salaire") != null &&
                                request.getParameter("idQualite") != null && request.getParameter("nombre") != null){

                           EmployeTravaillant appel = new EmployeTravaillant(request.getParameter("idQualite") , request.getParameter("idPersonnel") , request.getParameter("salaire") , request.getParameter("mettre") ,request.getParameter("nombre"));
                           appel.InsertEmployeTravaillant(null);
                        }
                    %>
                </div>
