...
<head>
    <%
        ...
        Materiel appelMateriel = new Materiel();
        List<Materiel> ListeMateriel = appelMateriel.SelectMaterielTsotra();
...
<body>
    ...
    <div class="container-fluid my-5">
            ...
            <div class="row justify-content-center w-100">
                <div class="col-md-12">
                    <h3>Inserer une materiel/Type</h3>
                    <form action="InsertionMaterielType.jsp" method="post" class="question-form w-100">
                        <div class="mb-3">
                            <select name="idType" class="form-control" required>
                                <% for (int i = 0; i < ListeType.size(); i++) { %>
                                    <option value="<%=ListeType.get(i).getIdType() %>"><%= ListeType.get(i).getNomType() %></option>
                                <% } %>
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <% for (int i = 0; i < ListeMateriel.size(); i++) { %>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="idMateriel[]" value="<%= ListeMateriel.get(i).getIdMateriel() %>">
                                    <label class="form-check-label w-100">
                                        <%= ListeMateriel.get(i).getNomMateriel() %> ( m2/unite )  
                                        <input type="text" class="form-control d-inline" name="quantite[]" value="0" w-75  > 
                                    </label>
                                </div>
                            <% } %>
                        </div>
                    ...
                    <%
                        if(request.getParameter("idType") != null && request.getParameter("idMateriel[]")!=null){

                            String[] idMaterielValues = request.getParameterValues("idMateriel[]");
                            String[] quantiteValue = request.getParameterValues("quantite[]");
                            int i = 0;
                            for (String idMateriel : idMaterielValues) {
                                MaterielType matType = new MaterielType(request.getParameter("idType"),idMateriel,quantiteValue[i],request.getParameter("idQualite"));
                                i++;
                                matType.InsertMaterielType(null);
                            }
                        }
                    %>
                </div>
